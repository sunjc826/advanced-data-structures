// A queue using the memory allocator instead of directly invoking stlib's malloc and free
// QueueNode_return(node) is always able to replace free(node)
// and node = QueueNode_get() is always able to replace malloc(sizeof(QueueNode))
#include "common.h"
...
static void Queue_free(QueueNode *queue)
{
    ...
    // Note that it is semantically wrong to use QueueNode_free(queue) here
    // Because by my convention, XXXStruct_free will free the members of XXXStruct and *XXXStruct
    // Since Queue_free_members has freed the members of *queue, using QueueNode_free will free the members twice
    // which is incorrect.
    QueueNode_return(queue);
}
